...
@Aspect
@Component
public class MyProductsServiceLoggingAspect {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(MyProductsConrollerLoggingAspect.class);
	
	@Before("execution(* com.example.ProductHub.Service.MyProductsService.*(..))")
    public void logBeforeMethod(JoinPoint joinPoint) {
        LOGGER.info("Entering method: " + joinPoint.getSignature().getName() + " with arguments: " + joinPoint.getArgs());
    }

    @AfterReturning(pointcut = "execution(* com.example.ProductHub.Service.MyProductsService.*(..))", returning = "result")
    ...
    @AfterThrowing(pointcut = "execution(* com.example.ProductHub.Service.MyProductsService.*(..))", throwing = "exception")
